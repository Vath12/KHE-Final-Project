...
# Set up page
st.set_page_config(
    layout="wide", page_title=f"Course: {class_info['name']} - Announcements"
)
...
with col_left:
    st.markdown(
        f"<p style='color: #7F27FF; font-size: 2.5em;'>{class_info['name']}: Announcements</p>",
    ...
    with st.expander("Create New Announcement", expanded=False):
        with st.form("create_announcement_form"):
            new_title = st.text_input(
                "Announcement Title", placeholder="Enter a title for your announcement"
            )
                ...
                if not new_title or not new_message:
                    ...
                    try:
                        result = createAnnouncement(
                            st.session_state.selected_class_id, new_title, new_message
                        )
    ...
    for i, announcement in enumerate(announcements):
        ...
        if not announcement_id:
            with st.container(border=True):
                ...
                st.markdown(
                    f"<p style='font-size: 0.9rem; color: #666;'>Posted: {date}</p>",
                    unsafe_allow_html=True,
                )
        ...
        if st.session_state.editing_announcement == announcement_id:
            ...
            with st.container(border=True):
                if is_instructor:
                    ...
                    with col2:
                        if st.button(
                            "✏️", key=f"edit_{i}", help="Edit this announcement"
                        ):
                    ...
                    with col3:
                        if st.button(
                            "🗑️", key=f"delete_{i}", help="Delete this announcement"
                        ):
                            st.warning(
